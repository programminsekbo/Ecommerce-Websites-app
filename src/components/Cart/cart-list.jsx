                                ...
                                <ul className="list-group list-group-flush">
                                    {
                                        CartList.map((item,i)=>{
                                            ...
                                            return(<li key={i} className="list-group-item d-flex justify-content-between align-items-start">
                                                    <img className="rounded-1" width="90" height="auto" src={item['product'][0]['image']}/>
                                                    <div className="ms-2 me-auto">
                                                        <p className="fw-lighter m-0">{item['product'][0]['title']}</p>
                                                        <p className="fw-lighter my-1">Unit Price: {price},Qty: {item['qty']}, Size: {item['size']}, Color: {item['color']}</p>
                                                        <p className=" h6 fw-bold m-0 text-dark">Tota  <i className="bi bi-currency-dollar"></i>{parseInt(price)*parseInt(item['qty'])}</p>
                                                    </div>
